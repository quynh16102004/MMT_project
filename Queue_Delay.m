...
%tim tre cua cap node o tai thap
    ...
    if ~isempty(idx1)
        delay_matrix(i, 1) = mean(DATA_LOW(idx1, 5) - DATA_LOW(idx1, 3)); 
    end
%tim tre cua cap node o tai cao
    idx2 = find(DATA_MED(:, 1) == src & DATA_MED(:,2) == dst);
    if ~isempty(idx2)
        delay_matrix(i, 2) = mean(DATA_MED(idx2, 5) - DATA_MED(idx2, 3)); 
    end

    idx3 = find(DATA_HIGH(:,1) == src & DATA_HIGH(:,2) == dst);
    if ~isempty(idx3)
        delay_matrix(i,3) = mean(DATA_HIGH(idx3, 5) - DATA_HIGH(idx3, 3)); 
    end
end
...
title({'MÔ PHỎNG BIẾN ĐỘNG TRỄ HÀNG ĐỢI THEO CÁC MỨC TẢI NGHẼN MẠNG'}, ...
